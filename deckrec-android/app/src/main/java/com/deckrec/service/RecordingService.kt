    ...
    override fun onCreate() {
        ...
        combine(app.recordingEngine.progress, app.recordingEngine.state) { progress, state ->
            ...
            .onEach { (progress, state) ->
                ...
                val finished = state is RecorderState.Failed ||
                    (state is RecorderState.Idle && sawRecording)
                if (finished) {
                    stopEverything()
                    return@onEach
                }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ACTION_START -> {
                ...
                acquireWakeLock()
                if (!app.recordingEngine.isRecording) {
                    app.recordingEngine.start(config)
                }
            }

            ACTION_MARK -> app.recordingEngine.addMarker()

            ACTION_TOGGLE_PAUSE -> {
                app.recordingEngine.togglePause()
                val state = app.recordingEngine.state.value
                val progress = app.recordingEngine.progress.value
                notifyIfPermitted(
                    buildNotification(state, progress.elapsedMs, progress.sizeBytes, progress.markerCount)
                )
            }

            ACTION_STOP -> {
                // stop() joins the audio thread, so it must not run on the main thread.
                scope.launch {
                    withContext(Dispatchers.IO) { app.recordingEngine.stop() }
                    stopEverything()
                }
            }
        }
